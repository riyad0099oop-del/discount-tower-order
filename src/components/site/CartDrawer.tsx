          ...
          {cart.map((item) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1 flex flex-col justify-between py-1">
                ...
                {item.notes && (
                  <p className="truncate text-xs text-muted-foreground">ملاحظة: {item.notes}</p>
                )}

                <div className="mt-3 flex items-center justify-between">
                  <span className="font-black text-lg text-primary">
                    {item.unitPrice * item.qty} <span className="text-xs">ر.س</span>
                  </span>
        ...
        <div className="space-y-4 border-t border-border/50 bg-surface p-6 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.05)]">
          <div className="flex justify-between items-end text-lg font-black">
            ...
            <span className="text-2xl text-primary">
              {cartTotal} <span className="text-sm text-foreground">ر.س</span>
            </span>
          </div>
          <Button
            asChild
